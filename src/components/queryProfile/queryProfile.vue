      ...
      <a-page-header :title="$t('order.profile.work_id', { id: order.work_id })" @back="() => $router.go(-1)"
            ...
            <template #extra>
                  <template v-if="order.status === 1">
                        <a-button key="2" danger ghost
                              @click="() => query.Reject(order.work_id).then(() => $router.go(-1))">{{
                                          $t('order.reject')
                              }}</a-button>
                        <a-button key="1" type="primary"
                              @click="() => query.Agree(order.work_id).then(() => $router.go(-1))">{{ $t('order.agree')
                              }}</a-button>
                  </template>
            </template>
            <a-row type="flex" justify="center" align="middle">
                  <a-col :span="16">
                        <a-descriptions :column="2">
                              <a-descriptions-item :label="$t('query.profile.username')">{{ order.username }}
                              </a-descriptions-item>
                              <a-descriptions-item :label="$t('query.profile.realname')">{{ order.real_name }}
                              </a-descriptions-item>
                              <a-descriptions-item :label="$t('query.profile.export')">{{ order.export === 1 ?
                                          $t('common.yes') : $t('common.no')
                              }}</a-descriptions-item>
                              <a-descriptions-item :label="$t('query.profile.date')">{{ order.date }}
                              </a-descriptions-item>
                              <a-descriptions-item :label="$t('query.profile.text')">
                                    <pre>{{ order.text }}</pre>
                              </a-descriptions-item>
                              <a-descriptions-item :label="$t('query.profile.approval')">{{ order.approval_time }}
                              </a-descriptions-item>
                        </a-descriptions>
                  </a-col>
                  <a-col :span="8">
            ...
            <a-tabs>
                  <a-tab-pane key="1" :tab="$t('order.profile.profile')">
                        <a-table bordered :columns="col" :dataSource="tData" size="small" :pagination="false"></a-table>
                        <br />
                        <a-pagination :total="pagination.pageCount" :page-size.sync="pagination.pageSize"
                              :show-total="(total) => `共 ${total} 条`" v-model:current="page" @change="currentPage" />
                  </a-tab-pane>
            </a-tabs>
      </a-page-header>
</template>
