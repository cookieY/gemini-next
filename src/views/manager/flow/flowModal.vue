<template>
  ...
  >
    <a-row>
      <a-col :span="10">
        <a-form ref="formRef" :model="step" :rules="rules">
          <a-form-item
            :label="$t('menu.manage.flow') + $t('common.table.name')"
            required
          >
          ...
          <a-form-item
            :label="$t('flow.step') + $t('common.table.name')"
            name="desc"
          >
            <a-input v-model:value="step.desc"></a-input>
          </a-form-item>
          <a-form-item
            :label="$t('flow.step') + $t('common.table.type')"
            name="type"
          >
      ...
      <a-col :span="13" :offset="1">
        <a-steps direction="vertical" progress-dot size="small" :current="0">
          <a-step
            ...
            :title="`${element.desc} ${
              element.type === 1
                ? `(${$t('flow.execute') + ' ' + $t('flow.phase')})`
                : element.type === 0
                ? `(${$t('flow.audit') + ' ' + $t('flow.phase')})`
                : ''
            }`"
            class="empty"
          >
...
<script lang="ts" setup>
  ...
  const rules = {
    desc: [
      { required: true, trigger: 'blur', message: t('common.check.flow') },
    ],
    type: [
      { required: true, trigger: 'change', message: t('common.check.type') },
    ],
    auditor: [
      { required: true, trigger: 'change', message: t('common.check.auditor') },
    ],
  };
  ...
  const upward = (idx: number) => {
    ...
    flow.value.steps[idx] = flow.value.steps.splice(
      idx - 1,
      1,
      flow.value.steps[idx]
    )[0];
  };
